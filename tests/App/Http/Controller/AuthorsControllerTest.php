...
class AuthorsControllerTest extends TestCase
{
    ...
    public function validation_invalidates_name_when_name_is_just_too_long()
    {
        foreach ($this->getValidationTestData() as $test) {
            ...
            $this->assertEquals(["The name must not be greater than 255 characters."], $data['name']);
        }
    }
    ...
    public function store_returns_a_valid_location_header()
    {
        ...
        $id = $data['data']['id'];
        $this->assertMatchesRegularExpression("#/authors/{$id}$#", $this->response->headers->get('Location'));
    }
